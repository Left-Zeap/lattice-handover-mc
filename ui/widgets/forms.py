...
_GROUP_SPECS: tuple[tuple[str, tuple[FieldSpec, ...]], ...] = (
    (
        ...
        (
            ...
            FieldSpec("detuning_ghz", "D1 红失谐 (GHz)", "double",
                      1.0, 5000.0, 1, 10.0),
            FieldSpec("source_power_w", "L1/L2 固定源端功率/分支 (W)",
                      "double", 0.0, 50.0, 3, 0.1),
            FieldSpec("delivery_efficiency", "源端→原子处单程传输效率", "double",
                      0.01, 1.0, 3, 0.05),
            FieldSpec("retro_power_ratio", "原子处回程/前向功率比", "double",
                      0.0, 1.0, 8, 0.01),
    ...
    (
        ...
        (
            ...
            FieldSpec("l1_maximum_velocity_m_s", "最大速度 (m/s)", "double",
                      0.1, 50.0, 2, 0.1),
            FieldSpec("l1_start_beam_diameter_um",
                      "L1 起点光束直径 2w (µm)", "double",
                      20.0, 4000.0, 1, 10.0),
            FieldSpec("l1_minimum_waist_um",
                      "L1 最小束腰半径 w₀ (µm)", "double",
                      10.0, 2000.0, 1, 10.0),
            FieldSpec("l1_minimum_waist_position_m",
                      "L1 最小束腰位置（距起点 m）", "double",
                      0.001, 4.999, 3, 0.01),
    ...
    (
        ...
        (
            ...
            FieldSpec("particle_count", "MC 代表粒子数 N", "int",
                      1, 1000000, 0, 100),
            FieldSpec("time_step_us", "handover 积分步长 (µs)", "double",
                      0.01, 100.0, 2, 0.05),
            FieldSpec("trace_points", "输出轨迹采样点数", "int", 2, 1001, 0, 2),
            ...
            FieldSpec("transport_time_step_us", "L1/L2 MC 积分步长 (µs)",
                      "double", 0.01, 100.0, 2, 0.05),
            FieldSpec("compute_backend", "数值计算设备", "combo",
                      options=(
                          ("cpu", "CPU（默认）"),
                          ("gpu", "GPU（CuPy/CUDA，需安装）"),
                      )),
            FieldSpec("parallel_backend", "二维扫描并行方式", "combo",
                      options=(
                          ("serial", "串行"),
                          ("process", "进程池（大网格推荐）"),
                      )),
            FieldSpec("worker_count", "二维扫描进程数", "int", 1, 128, 0, 1),
        ),
    ),
    (
        "L2 段",
